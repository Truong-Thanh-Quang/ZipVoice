...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
      ...
      hf_repo=hynt/ZipVoice-Vietnamese-2500h
      mkdir -p ${download_dir}
      for file in iter-525000-avg-2.pt tokens.txt config.json; do
            hf download \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
      ...
      python3 -m zipvoice.bin.train_zipvoice \
            ...
            --max-duration 60 \
            --max-len ${max_len} \
            --model-config ${download_dir}config.json \
            --checkpoint ${download_dir}iter-525000-avg-2.pt \
            --tokenizer ${tokenizer} \
            --lang ${lang} \
            --token-file ${download_dir}tokens.txt \
            --dataset custom \
